...
exports.createThread = exports.addPost = exports.getThread = exports.getThreads = exports.getBoard = exports.getBoards = exports.MOCK_BOARDS = void 0;
exports.MOCK_BOARDS = [
    // --- ラウンジ ---
    {
        id: "lounge-live",
...
const getBoards = async () => {
    return exports.MOCK_BOARDS;
};
exports.getBoards = getBoards;
const getBoard = async (boardId) => {
    return exports.MOCK_BOARDS.find((b) => b.id === boardId);
};
exports.getBoard = getBoard;
// --- Logic for Growth & Selection ---
function calculateMomentum(thread) {
    const now = new Date();
...
function checkSelectionStatus(thread) {
    const now = new Date();
    const created = new Date(thread.createdAt);
...
const getThreads = async (boardId) => {
    const board = exports.MOCK_BOARDS.find((b) => b.id === boardId);
    if (!board)
        return [];
...
const getThread = async (boardId, threadId) => {
...
const addPost = async (boardId, threadId, post) => {
    const thread = await (0, exports.getThread)(boardId, threadId);
    if (!thread)
        return undefined;
    const newPost = {
        id: `post-${Date.now()}`,
...
const createThread = async (boardId, title, firstPostContent, userId) => {
    const board = await (0, exports.getBoard)(boardId);
    if (!board)
        return undefined;
